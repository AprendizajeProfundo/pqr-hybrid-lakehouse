# 📖 Explicación Profunda: Golden Schema, SLA vs SLO
...
CREATE TABLE gold_kpi_backlog_daily (
  day DATE,
  pqrs_type VARCHAR(1),
  region VARCHAR(50),          -- 'Bogotá', 'Cali', 'Medellín', etc. (`region` viene de `dim_geo`, que ahora contiene códigos
  DANE y geometría para mapas avanzados)
...
**¿Qué mide?**
- El número de tickets **abiertos/pendientes** al final del día, por tipo y región.  La región
  se toma de la dimensión `dim_geo`, que almacena códigos DANE y coordenadas,
  permitiendo luego construir **mapas de calor** o calcular distancias según
  la geometría.
